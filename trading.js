(function () {
  ...
  var accounts = JSON.parse(
    localStorage.getItem("kyles-paper-accounts") || "{}",
  );
  var state =
  ...
  var markets = {
    ...
    crypto: {
      pairs: {
        ...
        ETHUSD: {
          title: "ETH / USD",
          symbol: "COINBASE%3AETHUSD",
          price: 3480,
          decimals: 2,
        },
        SOLUSD: {
          title: "SOL / USD",
          symbol: "COINBASE%3ASOLUSD",
          price: 142,
          decimals: 2,
        },
      },
  ...
  var savedBotNames = JSON.parse(
    localStorage.getItem("kyles-paper-bot-names") || "[]",
  );
  var botFirstNames = [
    "velvet",
    "pixel",
  ...
  while (botNames.length < 100) {
    var generatedName =
      botFirstNames[Math.floor(Math.random() * botFirstNames.length)] +
      botSecondNames[Math.floor(Math.random() * botSecondNames.length)] +
  ...
  function market() {
    return markets[activeMarket].pairs[currentPair];
  }
  function save() {
    if (!username) return;
    ...
    if (window.firebaseCloud) {
      window.firebaseCloud.publish(username, state).catch(function (error) {
  ...
  function renderLeaderboard() {
    ...
    Object.keys(accounts).forEach(function (accountName) {
      if (accounts[accountName].day !== today) {
        accounts[accountName].day = today;
        accounts[accountName].dailyEarnings = 0;
      }
    });
    var combinedAccounts = Object.assign({}, accounts, cloudAccounts);
    var entries = Object.keys(combinedAccounts)
      .map(function (accountName) {
        return { name: accountName, data: combinedAccounts[accountName] };
      })
      .sort(function (a, b) {
        return b.data.dailyEarnings - a.data.dailyEarnings;
      });
    leaderboardEmpty.hidden = entries.length > 0;
    leaderboardList.innerHTML = entries
      .map(function (entry, index) {
        var result = entry.data.dailyEarnings || 0;
        var avatar = entry.data.profilePic
          ? '<img class="profile-avatar" src="' +
            entry.data.profilePic +
            '" alt="" />'
          : '<i class="profile-avatar avatar-circle">' +
            entry.name.charAt(0).toUpperCase() +
            "</i>";
        var botLabel = entry.data.isBot
          ? " <small>BOT / " + entry.data.lastTrade + " TRADE</small>"
          : "";
        return (
          '<div class="leaderboard-row"><strong>' +
          (index + 1) +
          "</strong><span>" +
          avatar +
          entry.name +
  ...
  function openProfitLoss() {
    return state.trades.reduce(function (total, trade) {
      var livePrice =
        trade.market === activeMarket && trade.pairKey === currentPair
          ? currentPrice
          : markets[trade.market].pairs[trade.pairKey].price;
  ...
  function updatePortfolio() {
    ...
    weeklyEarnings.textContent =
      (state.weeklyEarnings >= 0 ? "+" : "") + money(state.weeklyEarnings);
    ...
    tradesList.innerHTML = state.trades
      .map(function (trade) {
        var direction = trade.side === "buy" ? 1 : -1;
        var livePrice =
          trade.market === activeMarket && trade.pairKey === currentPair
        ...
        return (
          ...
          trade.units +
          " units at " +
          trade.entry.toFixed(trade.decimals) +
          "</span><em>" +
          (liveResult >= 0 ? "+" : "") +
          money(liveResult) +
          "</em></li>"
  ...
  function runBotTrading() {
    botNames.forEach(function (botName) {
      ...
      var tradeSize =
        Math.random() < 0.025
          ? 40000000
          : Math.floor(100000 + Math.random() * 9000000);
  ...
  function updateChart() {
    chart.src =
      ...
      market().symbol +
      "&interval=15&hidesidetoolbar=1&symboledit=1&saveimage=0&toolbar_bg=%23080808&studies=%5B%5D&theme=dark&style=1&timezone=Etc%2FUTC&withdateranges=1&hideideas=1&hide_top_toolbar=1&hide_legend=0&hide_volume=1&allow_symbol_change=0&calendar=0";
  }
  function selectMarket(name) {
    ...
    terminal.classList.toggle(
      "is-hidden",
      isPortfolio || isLeaderboard || isSignup || isSettings || !username,
    );
    ...
    if (!isPortfolio && !isLeaderboard && !isSignup && !isSettings) {
      ...
      pairSelector.innerHTML = Object.keys(markets[name].pairs)
        .map(function (pair) {
          return (
            '<option value="' +
            pair +
            '">' +
            markets[name].pairs[pair].title +
            "</option>"
          );
        })
        .join("");
    ...
    document.querySelectorAll(".terminal-tab").forEach(function (tab) {
      tab.classList.toggle("is-active", tab.dataset.market === name);
    });
    updatePortfolio();
  }
  document.querySelectorAll(".terminal-tab").forEach(function (tab) {
    tab.addEventListener("click", function () {
      selectMarket(tab.dataset.market);
    });
  });
  ...
  loginForm.addEventListener("submit", function (event) {
    ...
    if (
      !account ||
      account.password !== document.getElementById("login-password").value
    ) {
    ...
    if (state.day !== loginToday) {
      state.day = loginToday;
      state.dailyEarnings = 0;
      save();
    }
    localStorage.setItem("kyles-active-user", username);
  ...
  function checkAdminStatus() {
    ...
    const activeUser = localStorage.getItem("kyles-active-user");
    const adminPanel = document.getElementById("admin-panel");

    if (adminPanel) {
      if (isAdmin || activeUser === "kd") {
  ...
  document
    ...
    .addEventListener("submit", function (event) {
      ...
      document.getElementById("settings-message").textContent =
        "Log in before changing settings.";
      return;
    });
  ...
  var applyChanges = function (profilePic) {
    ...
    if (newName && newName !== username) {
      ...
      state.usernameChangedAt = new Date().toISOString();
      username = newName;
    }
    accounts[username] = state;
    localStorage.setItem("kyles-active-user", username);
  ...
  window.renderLeaderboard = function (users) {
    const leaderboardContainer = document.getElementById("leaderboard");
    const isAdmin = localStorage.getItem("isAdmin") === "true";
    if (!leaderboardContainer) return;

    leaderboardContainer.innerHTML = users
      .map(
        (user) => `
...
if (!localStorage.getItem("kyles-active-user")) {
